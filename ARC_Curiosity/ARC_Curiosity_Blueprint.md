...
![Curiosity Definition](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathbf{Curiosity}(\cdot)%20\;=\;%20\underbrace{\text{Novelty}}_{\text{we%20haven't%20seen%20this}}%20\times%20\underbrace{\text{Learnability}}_{\text{we%20can%20improve%20here}}%20\times%20\underbrace{\text{Usefulness}}_{\text{it%20helps%20future%20tasks}})
...
### A. Bayesian Surprise (any model or module ![M](https://latex.codecogs.com/svg.latex?\bg_white&space;M))
Given prior ![p(theta)](https://latex.codecogs.com/svg.latex?\bg_white&space;p(\theta)), posterior ![p(theta|D cup e)](https://latex.codecogs.com/svg.latex?\bg_white&space;p(\theta\mid%20\mathcal{D}\cup\{e\})) after evidence ![e](https://latex.codecogs.com/svg.latex?\bg_white&space;e):
![Bayesian Surprise](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{Surprise}_M(e)%20\;=\;%20\mathrm{KL}\!\left[p(\theta\mid%20\mathcal{D}\cup\{e\})%20\,\|\,%20p(\theta\mid%20\mathcal{D})\right])
...
![Epistemic Uncertainty](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{Var}_{\text{epistemic}}\!\left[\mathrm{score}(h)\right])
High variance flags promising knowledge gaps.

### C. Learning Progress (LP)
For any metric ![m](https://latex.codecogs.com/svg.latex?\bg_white&space;m) (accuracy, solve rate, time-to-solve, stability variance):
![Learning Progress](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{LP}(t)%20\;=\;%20m(t)%20-%20m(t-\Delta))
...
![Information Gain](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{IG}%20\;=\;%20\mathbb{E}_{\text{outcome}}\!\left[\mathrm{KL}\big(p(\phi%20\mid%20\text{outcome})%20\,\|\,%20p(\phi)\big)\right])

where ![phi](https://latex.codecogs.com/svg.latex?\bg_white&space;\phi) are parameters of proposer/critic/executor.

### E. Empowerment (controllability)
Maximize mutual information between actions ![A](https://latex.codecogs.com/svg.latex?\bg_white&space;A) and reachable states ![S](https://latex.codecogs.com/svg.latex?\bg_white&space;S):
![Empowerment](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{Empower}(s)%20\;\approx\;%20I(A;%20S'%20\mid%20S=s))
...
![Task Curiosity Score](https://latex.codecogs.com/svg.latex?\bg_white&space;C_{\text{task}}(\tau)%20\;=\;%20\alpha\,\mathrm{IG}_{\text{solver}}(\tau)%20\;+\;%20\beta\,\mathrm{Surprise}_{\text{prior}}(\tau)%20\;+\;%20\gamma\,\mathrm{LP}_{\text{forecast}}(\tau)%20\;-\;%20\delta\,\mathrm{Redundancy}(\tau))
...
![UCB Formula](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{UCB}_k%20\;=\;%20\hat{\mu}_k%20\;+\;%20c\sqrt{\frac{\ln%20N}{n_k}})

where arm ![k](https://latex.codecogs.com/svg.latex?\bg_white&space;k) is a schema bucket, ![mu_k](https://latex.codecogs.com/svg.latex?\bg_white&space;\hat{\mu}_k) = recent LP, ![n_k](https://latex.codecogs.com/svg.latex?\bg_white&space;n_k) pulls, ![N](https://latex.codecogs.com/svg.latex?\bg_white&space;N) total pulls.

---

### 3.2 Workspace (the Conductor): *Which hypothesis is worth thinking about next?*

For each candidate hypothesis ![h](https://latex.codecogs.com/svg.latex?\bg_white&space;h) (program edit / parameter set):
![Hypothesis Score](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{Score}(h)%20\;=\;%20\underbrace{\mathrm{Fit}(h)}_{\text{critic}}%20\;-\;%20\lambda%20\underbrace{\mathrm{Instability}(h)}_{\text{navigator}}%20\;+\;%20\eta%20\underbrace{\mathrm{Curiosity}(h)}_{\text{below}})
with

![Hypothesis Curiosity](https://latex.codecogs.com/svg.latex?\bg_white&space;\mathrm{Curiosity}(h)%20\;=\;%20\alpha\,\mathrm{Var}_{\text{epistemic}}[\mathrm{Fit}(h)]%20\;+\;%20\beta\,\mathrm{IG}(h)%20\;+\;%20\rho\,\mathrm{Empower}(h))
...
![Basin Curiosity](https://latex.codecogs.com/svg.latex?\bg_white&space;C_{\text{basin}}(b)%20\;=\;%20\underbrace{\exp\!\big(-\mathrm{Var}_{\text{stab}}(b)\big)}_{\text{stable}}%20\cdot%20\underbrace{\mathrm{Novelty}(b)}_{\text{rare%20motif}}%20\cdot%20\underbrace{\mathrm{LP}(b)}_{\text{recent%20gains}})

**Edge curiosity** for trying new transitions ![i to j](https://latex.codecogs.com/svg.latex?\bg_white&space;i\!\to\!j):
![Edge Curiosity](https://latex.codecogs.com/svg.latex?\bg_white&space;C_{i\to%20j}%20\;=\;%20\omega_1\,\mathrm{IG}_{i\to%20j}%20\;+\;%20\omega_2\,\mathrm{Surprise}_{i\to%20j}%20\;+\;%20\omega_3\,\mathrm{CoverageGap}_{\mathrm{schema}(j)})
...
![Unifying Objective](https://latex.codecogs.com/svg.latex?\bg_white&space;U%20\;=\;%20\underbrace{\mathbb{E}[\mathrm{SolveGain}]}_{\text{exploitation}}%20\;-\;%20\lambda%20\underbrace{\mathrm{Compute}}_{\text{budget}}%20\;-\;%20\mu%20\underbrace{\mathrm{Instability}}_{\text{navigator}}%20\;+\;%20\kappa%20\underbrace{\mathrm{Curiosity}}_{\text{IG/LP/Surprise}})

Tune ![lambda mu kappa](https://latex.codecogs.com/svg.latex?\bg_white&space;(\lambda,\mu,\kappa)) via meta-optimization; or adapt online with a **curiosity budget**.

---

## 5) The Curiosity Budget & Ledger (Practical Control)

- **Budget:** allocate a fraction ![B in 0,1](https://latex.codecogs.com/svg.latex?\bg_white&space;B\in[0,1]) of steps/evals to curiosity-driven choices. Start higher, anneal as confidence grows.  
...
- **Stability gate:** curiosity expansions only if predicted instability < ![tau](https://latex.codecogs.com/svg.latex?\bg_white&space;\tau), or run as **bounded probes** (tiny budget, hard stop).  
- **De-leak novelty:** compute novelty on **intrinsic** descriptors (I/O signatures, grammar stats), not post-hoc solved outputs.  
- **Reproducibility:** deterministic decoding in analysis passes; inject noise only via controlled ![epsilon](https://latex.codecogs.com/svg.latex?\bg_white&space;\varepsilon)-perturbations.  
...
- **Ablations:** remove IG, remove LP, remove empowerment, swap UCB with ![epsilon](https://latex.codecogs.com/svg.latex?\bg_white&space;\varepsilon)-greedy.
