...
A typed function:
![Node Function](https://latex.codecogs.com/svg.latex?\bg_white&space;f:%20x%20\;\mapsto\;%20(y,%20\%20\text{artifacts},%20\%20\text{telemetry}))
...
For each node ![i](https://latex.codecogs.com/svg.latex?\bg_white&space;i), build a vector by concatenating:
...
![Angle Formula](https://latex.codecogs.com/svg.latex?\bg_white&space;\theta(i,j)%20=%20\arccos\big(\operatorname{cos\_sim}(b_i,%20b_j)\big))
**Distance** can be **path length** with edge costs ![c(i,j)](https://latex.codecogs.com/svg.latex?\bg_white&space;c(i,j)=1-\operatorname{cos\_sim}(b_i,%20b_j)) or ![1/cos_sim](https://latex.codecogs.com/svg.latex?\bg_white&space;1/\operatorname{cos\_sim}) when appropriate.

### 5.3 Utility updates (causal credit)
Edges ![i to j](https://latex.codecogs.com/svg.latex?\bg_white&space;i%20\to%20j) accrue **causal credit** via counterfactuals:
![Causal Credit](https://latex.codecogs.com/svg.latex?\bg_white&space;\Delta%20u_{i\to%20j}%20\;=\;%20\Delta%20\text{success}\big(\text{using%20}%20i\!\to\!j%20\text{%20vs.%20skipping%20}%20j\big)%20\;-\;%20\lambda%20\cdot%20\text{variance})
...
5. **Stability meter** probes sensitivity via small ![epsilon](https://latex.codecogs.com/svg.latex?\bg_white&space;\varepsilon)-perturbations; controller prefers **low-variance basins**, explores alternatives when **chaos** is detected.  
...
Duplicate run ![K](https://latex.codecogs.com/svg.latex?\bg_white&space;K) times with ![epsilon](https://latex.codecogs.com/svg.latex?\bg_white&space;\varepsilon)-perturbations in early nodes; track divergence of final metrics (IoU, object mapping accuracy). A positive empirical “exponent” indicates **chaotic** region.
...
![Trajectory Vector](https://latex.codecogs.com/svg.latex?\bg_white&space;[%20\text{visited-nodes%20histogram}%20%20|%20\overline{\theta}%20\text{along%20path}%20%20|%20\text{cumulative%20utility}%20%20|%20\text{final%20scores}%20%20|%20\text{failure%20taxonomy}%20])
...
3. **Angles:** run each node on the probe bank; create **256-d behavior vectors** (primitive counts, success flags, output descriptors). Cosine for ![theta](https://latex.codecogs.com/svg.latex?\bg_white&space;\theta); costs ![cost formula](https://latex.codecogs.com/svg.latex?\bg_white&space;=%201%20-%20\cos).  
4. **Controller:** best-first over paths of length ![leq L](https://latex.codecogs.com/svg.latex?\bg_white&space;\le%20L) with **utility = predicted success − sensitivity penalty**; keep top-![k](https://latex.codecogs.com/svg.latex?\bg_white&space;k) trajectories (beam).  
5. **Stability meter:** for current best path, run **5 ![epsilon](https://latex.codecogs.com/svg.latex?\bg_white&space;\varepsilon)-perturbations** up front; if variance ![>tau](https://latex.codecogs.com/svg.latex?\bg_white&space;>\tau), **demote basin** and branch to nearest **low-variance junction**.  
