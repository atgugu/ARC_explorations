...
from solvers.curiosity_solver.solver import ARCCuriositySolver, visualize_grids
...
def _run_solver_on_task(solver, task, verbose=True):
    """Run solver on a single task (helper, not a pytest test)."""
...
def test_curiosity_signals():
    ...
    from solvers.curiosity_solver.curiosity.signals import CuriositySignals
...
def test_belief_dynamics():
    ...
    from solvers.curiosity_solver.belief_dynamics.belief_space import BeliefSpace, Hypothesis
...
def test_active_inference():
    ...
    from solvers.curiosity_solver.active_inference.engine import ActiveInferenceEngine
...
def main():
    ...
    for task in tasks:
        try:
            pred1, pred2 = _run_solver_on_task(solver, task, verbose=True)
            results.append({
                'task': task['name'],
                'success': True,
