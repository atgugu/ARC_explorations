...
Let ![X](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\mathcal{X}) denote the space of ARC grids and ![T](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\mathcal{T}) the space of **tasks** ![tau](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\tau), each a set of training pairs ![training pairs](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\{(x_i,y_i)\}_{i=1}^k) plus a test input ![x star](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}x^\star). A **task program** ![p in P](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p\in\mathcal{P}) maps inputs to outputs: ![y equals f_p(x)](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}y%20=%20f_p(x)). Our aim is to learn a **prior** ![p_theta](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p_\theta(p,%20L)) over programs ![p](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p) and object layouts ![L](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}L) (scene graphs), from observed tasks ![D](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\mathcal{D}), such that sampling ![sampling](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}(p,L)\sim%20p_\theta) and rendering yields **valid ARC-like tasks**. A solver ![S_phi](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}S_\phi) maps ![tau to y hat](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\tau%20\mapsto%20\hat{y}^\star). We seek a **closed loop**:

![Closed Loop](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\text{learn%20}%20p_\theta%20\%20\Rightarrow%20\%20\text{sample%20tasks%20}%20\tilde{\tau}%20\%20\Rightarrow%20\%20\text{train%20}%20S_\phi%20\%20\Rightarrow%20\%20\text{update%20}%20p_\theta%20\text{%20using%20solvability/novelty%20feedback})
...
- **Types.**
  - **Grid** ![G](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}G), **Object** ![O](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}O) (connected components), **Mask** ![M](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}M), **Color** ![C](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}C), **Relation** ![R](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}R), **Set** ![S](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}S).
- **Primitives.**  
...
- **Schemas (rule templates).**  
  - `copy_with_transform`: select ![S[O]](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}S[O]), apply ![T](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}T), place with constraint ![K](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}K).  
...
Each **schema** is a graph of primitives with typed edges; **parameters** ![psi](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\psi) (e.g., rotation ![k](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}k), axis, palette) and **selectors** ![sigma](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}\sigma) (which objects) are learned as random variables.

---

## 5. Generative Prior Model (GPM)

We model ![p_theta(p,L)](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p_\theta(p,%20L)) as a product of factors:

![GPM Factorization](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p_\theta(p,L)%20=%20p_\theta(L)\%20p_\theta(p\mid%20L))

where ![L](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}L) is a **scene graph** of object prototypes, spatial layouts, and palettes.

### 5.1 Scene Graph Generator ![p_theta(L)](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p_\theta(L))
...
### 5.2 Program Generator ![p_theta(p|L)](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}p_\theta(p\mid%20L))
...
- **Parameterization:** distributions over discrete/continuous parameters (e.g., rotation ![k in set](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}k%20\in%20\{0,1,2,3\})).  
- **Selector policies:** GNN computes attention over ![L](https://latex.codecogs.com/svg.latex?\pagecolor{white}\color{black}L) to pick objects by role.
